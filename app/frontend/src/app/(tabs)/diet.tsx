...
import { api, GeneratedRecipe, LoggedAtHomeMeal, LoggedMenuMeal } from '../../core/api'
...
export default function Diet() {
  ...
  const [dietView, setDietView] = useState<'classic' | 'enhanced'>(isWeb ? 'enhanced' : 'classic');
  ...
  async function handleAddGeneratedRecipe() {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Failed to add recipe to meal log.';
  ...
  async function handleAddRestaurantSuggestion(restaurant: string, order: string) {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Failed to add restaurant recommendation.';
          ...
          <View style={[styles.modalCard, styles.recipePromptCard]}>
            ...
            >
              <Text style={recipeGoal ? styles.selectInputText : styles.selectInputPlaceholder}>
                {recipeGoal ? formatLabel(recipeGoal) : 'Select goal'}
              </Text>
            </Pressable>
            {recipeDropdown === 'goal' ? (
              <View style={styles.dropdownList}>
                {GOALS.map((option: Goal) => (
                  <Pressable
                    key={option}
                    onPress={() => {
          ...
          <View style={[styles.modalCard, styles.recipeModalCard]}>
                ...
                {generatedRecipe.mode === 'restaurant' ? (
                  <>
                    ...
                    {(generatedRecipe.restaurant_suggestions ?? []).map((suggestion, index) => (
                      <View key={`${suggestion.restaurant}-${index}`} style={styles.restaurantSuggestionCard}>
            ...
            <View style={styles.modalButtonRow}>
              ...
              {generatedRecipe?.mode !== 'restaurant' ? (
                <View style={styles.modalButtonHalf}>
                  <ForgeButton
                    onPress={handleAddGeneratedRecipe}
...
const styles = StyleSheet.create({
  ...
  restaurantSuggestionTitle: {
    color: '#111827',
    fontSize: 15,
    fontWeight: '800',
  },
  restaurantSuggestionOrder: {
    color: '#1f2937',
    fontSize: 14,
    fontWeight: '700',
    lineHeight: 20,
  },
  restaurantSuggestionButton: {
    marginTop: 6,
  },
  modalButtonFull: {
    flex: 1,
